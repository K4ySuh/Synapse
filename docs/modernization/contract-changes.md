...
| `adr/ADR-0002-typed-core-action-registry.md` | same commit | Added descriptor-owned `intent_resolver` and one immutable execution plan shared by policy, executor, workers, and finalizers. | The Phase 1.1 review proved that effects alone did not expose deferred jobs, crawler scope expansion/output, redirects, or proxy routes. The amendment makes those execution dimensions enforceable without changing the frozen legacy wire surface or implementing grants. | Javier Roldán Ortiz |
