...
Replace caller-asserted `confirm=true` in the new application path with durable, server-evaluated Authority Grants carrying scope digest, allowed action patterns and methods, side-effect and risk ceilings, credential and provider restrictions, budgets, expiry, and revocation. `confirm=true` remains accepted only behind the explicitly selected legacy profile and is marked distinctly in audit records.
...
- Exercise expiry, revocation, scope digest changes, risk ceilings, credential
  restrictions, and third-party budgets before dispatch.
