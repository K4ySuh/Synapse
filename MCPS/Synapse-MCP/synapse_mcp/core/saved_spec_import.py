...
from .adapters import AdapterResult, WorkspaceEntityBundle, candidate_observation
from .errors import McpError
from .url_hygiene import is_high_entropy_value, is_sensitive_query_name
...
def _redact_value_preview(name: Any, value: Any, *, limit: int = 120) -> str:
    """Return a bounded preview without persisting credential-like values."""

    text = str(value)
    normalized = str(name).strip().lower().replace("_", "-")
    sensitive = (
        is_sensitive_query_name(normalized)
        or normalized in {"authorization", "cookie", "set-cookie"}
        or any(
    ...
    return "<redacted>" if text and (sensitive or is_high_entropy_value(text)) else text[:limit]
...
def _parameter(name: str, location: str, url: str, method: str, path: str, value_preview: str) -> dict[str, Any]:
    ...
    if value_preview:
        param["valuePreview"] = _redact_value_preview(name, value_preview)
    return param
