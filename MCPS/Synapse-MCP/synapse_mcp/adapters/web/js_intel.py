...
def _write_report_output(workspace_id: str, target: str, args: dict[str, Any], content: str, extension: str) -> Path:
    ...
    if output_path:
        ...
        if not _is_within_workspace(resolved, workspace_root) and args.get("allowExternalOutput") is not True:
            raise McpError(-32602, "External JS app-map output paths require allowExternalOutput=true.")
        resolved.parent.mkdir(parents=True, exist_ok=True)
        resolved.write_text(content, encoding="utf-8")
        return resolved
    path = workspace.workspace_path(workspace_id) / "reports" / f"js-app-map-{workspace.slug(target)}.{extension}"
