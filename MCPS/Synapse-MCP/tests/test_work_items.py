"""Service regression tests for work items."""
...
class WorkItemTests(unittest.TestCase):
    ...
    def test_existing_two_migration_database_upgrades_in_place_to_work_items(self) -> None:
        ...
        with upgraded.connection_factory.connect() as checked:
            ...
            self.assertEqual(int(checked.execute("SELECT COUNT(*) FROM schema_migrations").fetchone()[0]), 7)
    ...
    def test_missing_evidence_cannot_complete_or_mutate_work(self) -> None:
        item = self.create()
        claim = self.claim(item)
        with self.assertRaises(StateStoreError) as missing:
            self.repository.transition(
                item["workItemId"],
                {"resultSummary": "Unverified", "references": [{"type": "evidence", "id": "missing-evidence"}]},
                operation="complete",
